...
/**
 * Manages the mapping of file items using a single-threaded executor.
 */
object FileMapManager {
     ...
     *
     * @param items The list of root items to process.
     * @param priority Optional priority setting for the mapping thread.
     */
    fun startFileMapping(items: List<FileTreeItem>, priority: Int? = null) {
        if (fileCacheFuture == null) {
            fileCacheFuture = executor.submit {
                try {
                    val fileCache = ConcurrentFileMap(items)
                    fileCache.run() 
